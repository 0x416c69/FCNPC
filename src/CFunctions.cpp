...
void CFunctions::PlayerShoot(WORD wPlayerId, WORD wHitId, BYTE byteHitType, BYTE byteWeaponId, const CVector& vecPoint, const CVector& vecOffsetFrom, bool bIsHit, int iMode, BYTE byteBetweenCheckFlags)
{
    ...
    switch (bulletSyncDataTarget.byteHitType)
    {
        case BULLET_HIT_TYPE_NONE:
            if (bulletSyncDataTarget.wHitID >= 0 && bulletSyncDataTarget.wHitID < MAX_ACTORS)
            { // Actors don't have a hit type, this is conform with the SA-MP callback OnPlayerWeaponShot
            ...
            else if (bulletSyncDataTarget.wHitID == MAX_ACTORS + 1)
            { // Hit map
// logprintf("HIT MAP");
        ...
        case BULLET_HIT_TYPE_PLAYER:
            if (bulletSyncDataTarget.wHitID >= 0 && bulletSyncDataTarget.wHitID < MAX_PLAYERS)
            {
                ...
                if (pPlayer)
                {
                    ...
                    { // Separate code for players and NPCs, in case we need to handle them differently in the future
// logprintf("HIT PLAYER");
        ...
        case BULLET_HIT_TYPE_PLAYER_OBJECT:
            ...
            { // Player object IDs start at 1
                ...
                { // Handles player objects of the shooter
                ...
                else  if (wPlayerObjectOwnerId == wHitId)
                { // Handles player objects of the target
                ...
                {
                    if (!pServer->GetPlayerManager()->IsNPC(wPlayerObjectOwnerId))
                    { // Handles player objects of the closestPlayer
                    ...
                    { // Handles player objects of the closestNPC
                        CObject* pPlayerObject = pNetGame->pObjectPool->pPlayerObjects[wPlayerObjectOwnerId][bulletSyncDataTarget.wHitID];
                        if (pPlayerObject)
                        {
...
WORD CFunctions::GetClosestEntityInBetween(const CVector& vecHitOrigin, const CVector& vecHitTarget, float fRange, int iMode, BYTE byteBetweenCheckFlags, WORD wPlayerId, WORD wTargetId, BYTE& byteEntityType, WORD& wPlayerObjectOwnerId, CVector& vecHitMap)
{
    ...
    if (byteBetweenCheckFlags & ENTITY_CHECK_POBJECT_TARG)
...
WORD CFunctions::GetClosestVehicleInBetween(const CVector& vecHitOrigin, const CVector& vecHitTarget, float fRange, float& fDistance)
{
    ...
    for (WORD i = 1; i <= pNetGame->pVehiclePool->dwVehiclePoolSize; i++)
    { // Vehicle IDs start at 1
// Validate the vehicle
        ...
        if (CMath::GetDistanceFromRayToPoint(vecHitOrigin, vecHitTarget, pVehicle->vecPosition) > MAX_HIT_RADIUS_VEHICLE)
